...
void processTiles_weightStatinary(int numNeurons, int inputSize, int inputTileSize,
                                  std::vector<float> &weights, std::vector<float> &biases,
                                  std::vector<float> &inputs, std::vector<float> &outputs)
{
    std::cout << "Entered weightStatinary!...\n";
    cl_int err;
    int numTiles = inputSize / inputTileSize;
    outputs.assign(numNeurons, 0.0f);

    outputBuffer = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(float) * numNeurons, NULL, &err);
    if (err != CL_SUCCESS || outputBuffer == nullptr)
    {
        std::cerr << "Failed to create outputBuffer! OpenCL Error Code: " << err << std::endl;
        exit(EXIT_FAILURE);
    }
    float zero = 0.0f;
    clEnqueueFillBuffer(queue, outputBuffer, &zero, sizeof(float), 0, sizeof(float) * numNeurons, 0, NULL, NULL);
    for (int tile = 0; tile < numTiles; ++tile)
    {
        int offset = tile * inputTileSize;
        std::vector<float> inputTile(inputs.begin() + offset, inputs.begin() + offset + inputTileSize);
        std::vector<float> weightTile(numNeurons * inputTileSize);
        for (int i = 0; i < numNeurons; ++i)
            for (int j = 0; j < inputTileSize; ++j)
                weightTile[i * inputTileSize + j] = weights[i * inputSize + offset + j];

        inputTileBuffer = clCreateBuffer(context, CL_MEM_READ_ONLY, inputTileSize * sizeof(float), NULL, &err);
        if (err != CL_SUCCESS || inputTileBuffer == nullptr)
        {
            std::cerr << "Failed to create inputTileBuffer! Error: " << err << std::endl;
            exit(EXIT_FAILURE);
        }
        weightsTileBuffer = clCreateBuffer(context, CL_MEM_READ_ONLY, weightTile.size() * sizeof(float), NULL, &err);
        if (err != CL_SUCCESS || weightsTileBuffer == nullptr)
        {
            std::cerr << "Failed to create weightsTileBUffer! Error: " << err << std::endl;
            exit(EXIT_FAILURE);
        }
        clEnqueueWriteBuffer(queue, inputTileBuffer, CL_TRUE, 0, inputTileSize * sizeof(float), inputTile.data(), 0, NULL, NULL);
        clEnqueueWriteBuffer(queue, weightsTileBuffer, CL_TRUE, 0, weightTile.size() * sizeof(float), weightTile.data(), 0, NULL, NULL);
        if (!kernel)
        {
            std::cerr << "ERROR: Kernel is null - did init_opencl succeed?" << std::endl;
            exit(EXIT_FAILURE);
        }
        if (!inputTileBuffer)
        {
            std::cout << "inputTileBuffer is empty\n";
        }
        if (!weightsTileBuffer)
        {
            std::cout << "weightsTileBuffer is empty\n";
        }
        if (!outputBuffer)
        {
            std::cout << "outputBuffer is empty\n";
        }
        clSetKernelArg(kernel, 0, sizeof(cl_mem), &inputTileBuffer);
        clSetKernelArg(kernel, 1, sizeof(cl_mem), &weightsTileBuffer);
        ...
        size_t global_size[] = {static_cast<size_t>(numNeurons)};
        std::cout << "Launching kernel...\n";
        clEnqueueNDRangeKernel(queue, kernel, 1, NULL, global_size, NULL, 0, NULL, NULL);
        std::cout << "Launched kernel successfully...\n";
        clFinish(queue);
    ...
    clEnqueueReadBuffer(queue, outputBuffer, CL_TRUE, 0, sizeof(float) * numNeurons, outputs.data(), 0, NULL, NULL);
...
void run()
{
    std::cout << "Running on FPGA...\n";

    hidden_layer1_out.resize(numNeurons);
    output_layer_out.resize(numNeurons);

    std::cout << "{run} Resized output vectors...\n";
    processTiles_weightStatinary(numNeurons, inputSize, inputTileSize,
                                 hidden_layer1_weights, hidden_layer1_biases,
                                 image_data, hidden_layer1_out);
    std::cout << "[run] Calling weight statinary...\n";

    std::cout << "\nOutput before ReLU:\n";
    for (float val : hidden_layer1_out)
        std::cout << val << " ";
    std::cout << std::endl;
    ...
    processTiles_weightStatinary(numNeurons, numNeurons, 10,
                                 output_layer_weights, output_layer_biases,
                                 hidden_layer1_out, output_layer_out);

    std::cout << "\nOutput before log-softmax:\n";
    for (float val : output_layer_out)
        std::cout << val << " ";
    std::cout << std::endl;

    log_softmax(output_layer_out);

    std::cout << "\nOutput after log-softmax:\n";
    for (float val : output_layer_out)
        std::cout << val << " ";
    std::cout << std::endl;
    int predicted = getMaxIn(output_layer_out);
